...
function migrate(){
  ...
  S.debts.forEach(function(d){
    if (d.rate === undefined) d.rate = DEBT_RATE;
    if (d.cleared === undefined) d.cleared = false;
    if (d.retired === undefined) d.retired = 0;
    if (d.clearedDate === undefined) d.clearedDate = '';
  });
...
function debtStats(d){
  ...
  if (d.cleared) return { bal:0, ins:0, rate:r, delay:0, months:0,
                          cleared:true, unknown:false, stuck:false, retired:+d.retired||0 };
  var delay = bal * r;
  var months = (ins > 0 && bal > 0) ? Math.ceil(bal / ins) : 0;
  return { bal:bal, ins:ins, rate:r, delay:delay, months:months, cleared:false,
           unknown: bal <= 0, stuck: bal > 0 && ins <= 0, retired:+d.retired||0 };
}
function debtFlow(d){
...
function debtTotals(){
  var bal = 0, ins = 0, delay = 0, unknown = 0, longest = 0, stuck = 0,
      cleared = 0, retired = 0, live = 0;
  S.debts.forEach(function(d){
    var x = debtStats(d);
    retired += x.retired;
    if (x.cleared) { cleared++; return; }
    live++;
  ...
  return { bal:bal, ins:ins, delay:delay, unknown:unknown, longest:longest, stuck:stuck,
           count:S.debts.length, cleared:cleared, retired:retired, live:live };
}
...
function saveFunding(pid){
  ...
  p.funding = f;
  persist(); renderAll(); trace(pid); toast('Funding saved');
}
...
function saveTags(pid){
  ...
  persist(); renderAll(); trace(pid); toast('Outflows tagged');
}
...
function compTab(v){
  compView = v;
  var btns = document.querySelectorAll('#comp-seg button');
  btns[0].classList.toggle('on', v === 'nature');
  btns[1].classList.toggle('on', v === 'category');
  renderComposition();
}
...
function renderComposition(){
  ...
  c.innerHTML = keys.map(function(k){
    ...
    var fn = (compView === 'category') ? 'categoryDrill' : 'natureDrill';
    var link = ' tap" onclick="' + fn + '(\'' + k.replace(/'/g,"") + '\')"';
...
function natureDrill(nat){
  var t = totals();
  var items = S.flows.filter(function(f){ return (f.nature||'Untagged') === nat; })
                     .sort(function(a,b){ return flowM(b)-flowM(a); });
  var sum = items.reduce(function(x,f){ return x + flowM(f); }, 0);
  var blurb = { Fixed:'Committed regardless of choices made this month.',
    Variable:'Moves with usage, but expected.',
    Discretionary:'Entirely within your gift to cut.',
    Debt:'Balance-sheet work \u2014 retiring obligations, not consumption.',
    Investment:'Money placed to return later.',
    Untagged:'Give these a nature so the composition reads true.' }[nat] || '';
  openSheet('<h2>' + esc(nat) + '</h2><div class="sh-sub">' + items.length + ' outflow'
    + (items.length===1?'':'s') + ' \u00b7 ' + ugx(sum) + ' a month \u00b7 '
    + (t.out ? Math.round(sum/t.out*100) : 0) + '% of everything leaving</div>'
    + (blurb ? '<div class="note" style="margin-bottom:12px">' + blurb + '</div>' : '')
    + '<div class="card">' + items.map(function(f){
        ...
        return '<div class="row tap" onclick="openRef(\'flow\',\'' + f.id + '\')">'
          ...
          + (f.category ? ' \u00b7 ' + esc(f.category) : '') + '</div></div>'
...
function renderDebts(){
  ...
  el.innerHTML = '<div class="card-h"><div class="card-t">The position</div></div>'
    ...
    + '</div>'
    + (t.cleared ? '<div class="row"><div class="row-main"><div class="row-t">Cleared to date</div>'
        + '<div class="row-s">' + t.cleared + ' of ' + t.count + ' retired'
        + (t.retired ? ' \u00b7 ' + ugx(t.retired) + ' paid off' : '') + '</div></div>'
        + '<div class="row-v num" style="color:var(--good)">' + t.cleared + ' \u2713</div></div>' : '')
  ...
  var sorted = S.debts.slice().sort(function(a,b){
    var xa = debtStats(a), xb = debtStats(b);
    if (xa.cleared !== xb.cleared) return xa.cleared ? 1 : -1;
    return xb.bal - xa.bal;
  });
  list.innerHTML = '<div class="sec">Each obligation</div><div class="card">' + sorted.map(function(d){
    var x = debtStats(d);
    var pill = x.cleared ? '<span class="pill p-good">\u2713 Cleared' + (d.clearedDate ? ' ' + d.clearedDate.slice(5) : '') + '</span>'
             : x.unknown ? '<span class="pill p-gold">Balance not set</span>'
...
function payDebt(id){
  ...
  var x = debtStats(d);
  openSheet('<h2>Record a payment</h2><div class="sh-sub">' + esc(d.name)
    + (x.bal ? ' \u00b7 ' + ugx(x.bal) + ' outstanding' : ' \u00b7 balance not yet entered') + '</div>'
    + '<div class="f"><label>Amount paid (UGX)</label>'
    + '<input type="number" inputmode="numeric" id="e-pay" value="' + (x.ins||'') + '"></div>'
    + (x.bal ? '<div class="hint" style="margin-bottom:12px">Paying ' + ugx(x.bal)
        + ' clears it entirely.</div>' : '')
    + '<button class="btn au wide" onclick="savePayment(\'' + id + '\')">Record payment</button>'
    + (x.bal ? '<button class="btn wide" style="margin-top:9px" onclick="clearDebt(\'' + id
        + '\')">Mark fully cleared</button>' : '')
    + '<button class="btn ghost wide" style="margin-top:9px" onclick="editDebt(\'' + id + '\')">Back</button>');
}
function savePayment(id){
  var d = S.debts.find(function(x){ return x.id===id; });
  var amt = parseFloat(document.getElementById('e-pay').value);
  if (!amt || amt <= 0) { toast('Enter the amount paid'); return; }
  d.retired = (+d.retired||0) + amt;
  var hadBal = (+d.balance||0) > 0;
  d.balance = Math.max((+d.balance||0) - amt, 0);
  S.ledger.push({ id:uid(), ts:Date.now(), type:'debit', amount:amt, currency:'UGX',
    projectId:(debtFlow(d)||{}).projectId || '', note:'Payment \u2014 ' + d.name });
  if (hadBal && d.balance === 0) {
    d.cleared = true;
    d.clearedDate = new Date().toISOString().slice(0,10);
    persist(); closeSheet(); renderAll();
    toast('\uD83C\uDF89 ' + d.name + ' \u2014 CLEARED');
  } else {
    persist(); closeSheet(); renderAll();
    toast('Payment recorded \u00b7 ' + (d.balance ? ugx(d.balance) + ' remains' : 'update the balance when known'));
  }
}
function clearDebt(id){
  var d = S.debts.find(function(x){ return x.id===id; });
  d.retired = (+d.retired||0) + (+d.balance||0);
  d.balance = 0; d.cleared = true;
  d.clearedDate = new Date().toISOString().slice(0,10);
  persist(); closeSheet(); renderAll();
  toast('\uD83C\uDF89 ' + d.name + ' \u2014 CLEARED');
}
function reopenDebt(id){
  var d = S.debts.find(function(x){ return x.id===id; });
  d.cleared = false; d.clearedDate = '';
  persist(); closeSheet(); renderAll(); toast('Reopened \u2014 set the balance owing');
}
...
function editDebt(id){
  ...
  openSheet('<h2>' + (id?'Edit debt':'New debt') + '</h2>'
    ...
    + (id && !d.cleared ? '<button class="btn wide" style="margin-bottom:9px" onclick="payDebt(\'' + id + '\')">Record a payment</button>' : '')
    + (id && d.cleared ? '<div class="verdict v-good" style="margin-bottom:12px"><b>Cleared'
        + (d.clearedDate ? ' on ' + d.clearedDate : '') + '.</b> '
        + (d.retired ? ugx(d.retired) + ' retired in total. ' : '')
        + 'This debt no longer counts anywhere.</div>'
        + '<button class="btn ghost wide" style="margin-bottom:9px" onclick="reopenDebt(\'' + id + '\')">Reopen this debt</button>' : '')
...
function saveSource(id){
  ...
  persist(); closeSheet(); renderAll(); toast(id?'Source updated':'Source added');
}
function delSource(id){
  ...
  persist(); closeSheet(); renderAll(); toast('Source deleted');
}
...
function saveFlow(id){
  ...
  persist(); closeSheet(); renderAll(); toast(id?'Outflow updated':'Outflow added');
}
function delFlow(id){
  S.flows = S.flows.filter(function(x){ return x.id!==id; });
  persist(); closeSheet(); renderAll(); toast('Outflow deleted');
}
...
function saveProject(id){
  ...
  persist(); closeSheet(); renderAll(); toast(id?'Project updated':'Project added');
}
function delProject(id){
  ...
  persist(); closeSheet(); renderAll(); toast('Project deleted');
}
...
function saveReserve(id){
  ...
  persist(); closeSheet(); renderAll(); toast(id?'Reserve updated':'Reserve added');
}
function delReserve(id){
  S.reserves = S.reserves.filter(function(x){ return x.id!==id; });
  persist(); closeSheet(); renderAll(); toast('Reserve deleted');
}
...
function saveEntry(){
  ...
  persist(); closeSheet(); renderAll(); toast('Movement logged');
}
function delEntry(id){
  S.ledger = S.ledger.filter(function(x){ return x.id!==id; });
  persist(); renderAll(); toast('Movement deleted');
}
...
function saveSettings(){
  ...
  persist(); renderAll(); toast('Period saved');
}
...
function importJSON(input){
  ...
  r.onload = function(){
    try {
      ...
      S = d; if (S.v !== 3) S.v = 3;
      persist(); closeSheet(); renderAll(); toast('Backup restored');
...
function resetAll(){
  ...
  S = seed(); persist(); closeSheet(); renderAll(); toast('Reset to seed');
}
...
function saveCats(){
  ...
  persist(); closeSheet(); renderAll(); toast('Categories saved');
}
...
function renderAll(){
  var warnEl = document.getElementById('storage-warn');
  if (warnEl) warnEl.style.display = storageOK() ? 'none' : '';
...
/* ============================================================
   BOOT — runs once, at the end of the file
   ============================================================ */
migrate();
(function(){
  var bare = !S.sources.length && !S.flows.length && !S.projects.length && !S.debts.length;
  if (!bare) {
    persist(); renderAll(); handleShare();
    return;
  }
  /* Booted empty: consult the IndexedDB mirror BEFORE persisting anything,
     or the empty seed would overwrite the surviving copy. */
  renderAll(); handleShare();
  idb.read(function(v){
    var adopted = false;
    if (v) {
      try {
        var d = JSON.parse(v);
        if (d && d.sources &&
            (d.sources.length + d.flows.length + d.projects.length + (d.debts||[]).length) > 0) {
          S = d; migrate(); adopted = true;
        }
      } catch(e){}
    }
    persist();
    if (adopted) { renderAll(); toast('Recovered your data from the mirror store'); }
  });
})();
