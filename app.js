...
function trace(pid){
  ...
  own.forEach(function(s){
    h += '<div class="tr-item"><span class="tr-name">' + esc(s.name)
       + ' <span class="mini">· earns</span></span>'
       + '<span class="tr-amt pos">' + ugx(srcM(s)) + '</span></div>';
  });
  allocs.forEach(function(a){
    var s = S.sources.find(function(x){ return x.id === a.sourceId; });
    h += '<div class="tr-item"><span class="tr-name">' + esc(s ? s.name : 'Unknown source')
  ...
  outs.forEach(function(f){
    h += '<div class="tr-item"><span class="tr-name">' + esc(f.name)
       + (f.nature ? ' <span class="mini">· ' + esc(f.nature) + '</span>' : '')
       + '</span><span class="tr-amt neg">' + ugx(flowM(f)) + '</span></div>';
  });
  h += '</div></div>';
...
function renderPosition(){
  ...
  document.getElementById('alerts-card').innerHTML = buildAlerts().map(function(a){
    return '<div class="alert"><span class="dot" style="background:' + a.c + '"></span>'
      + '<span class="tx"><b>' + a.t + '</b><span>' + a.s + '</span></span></div>';
...
function buildAlerts(){
  ...
  if (cover < 3) a.push({c: cover < 1 ? RED : AU, t:'Reserve cover ' + cover.toFixed(1) + ' months',
    s:'Three months is the floor. You hold ' + ugx(resBal) + ' against ' + ugx(t.out) + ' of monthly outflow.'});
  else a.push({c:GR, t:'Reserve cover ' + cover.toFixed(1) + ' months', s:'At or above the three-month floor.'});

  var reqTotal = S.reserves.reduce(function(x,r){ return x + reserveMonthly(r); }, 0);
  if (reqTotal > t.net && reqTotal > 0) a.push({c:AU, t:'Goals outrun your surplus',
...
function renderComposition(){
  ...
  c.innerHTML = keys.map(function(k){
    ...
    return '<div class="row"><div class="row-main"><div class="row-t">' + esc(k) + '</div>'
...
function renderSources(){
  ...
  el.innerHTML = summary + '<div class="card">' + S.sources.map(function(s){
    ...
    return '<div class="row tap" onclick="editSource(\'' + s.id + '\')">'
...
function renderDebts(){
  ...
  list.innerHTML = '<div class="sec">Each obligation</div><div class="card">' + sorted.map(function(d){
    ...
    return '<div class="row tap" onclick="editDebt(\'' + d.id + '\')">'
      ...
      + (x.bal > 0 ? '<span>delay costs ' + ugx(x.delay) + '</span>' : '') + '</div></div>'
...
function renderAdvisor(){
  ...
  out.push({h:'The governing fact', b:
  ...
  /* 1b. the run rate */
  out.push({h:'What happens when the one-offs stop', b:
  ...
  if (dt.count) {
    out.push({h:'What the debts cost you', b: dt.bal
  ...
  if (silent.length) out.push({h:'Ventures not yet paying', b:
  ...
  /* 4. unassigned money */
  if (t.unassigned > 0) out.push({h:'Money you cannot account for', b:
  ...
  if (drains.length) out.push({h:'Largest net drain', b:
  ...
  /* 6. FX */
  if (t.usdOut > 0) out.push({h:'Currency risk', b:
  ...
  var resBal = S.reserves.reduce(function(x,r){ return x+(+r.balance||0); }, 0);
  var cover = t.out ? resBal/t.out : 0;
  out.push({h:'If income stopped tomorrow', b:
  ...
  if (camp) {
    ...
    out.push({h:'The 2031 horizon', b: need
  ...
  document.getElementById('adv-list').innerHTML = out.map(function(o,i){
    return '<div class="card"><div class="card-h"><div class="card-t">' + esc(o.h) + '</div>'
      + '<div class="mini num">' + String(i+1).padStart(2,'0') + '</div></div>'
      + '<div class="note">' + o.b + '</div></div>';
  }).join('');
}
...
function renderAll(){
  renderStrip();
  if (current === 'position') renderPosition();
  if (current === 'projects') renderProjects();
  if (current === 'money'){ renderSources(); renderFlows(); renderDebts(); renderLedger(); }
  if (current === 'reserves') renderReserves();
  if (current === 'advisor') renderAdvisor();
}
